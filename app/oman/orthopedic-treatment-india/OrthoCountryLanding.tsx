...
export default function OrthoCountryLanding({
  initialLang = "en",
}: {
  initialLang?: Lang;
}) {
  ...
  const orthoDoctors = doctors.filter((d) => d.treatmentSlug === "orthopedics");
  const orthoHospitalSlugs = new Set(orthoDoctors.map((d) => d.hospitalSlug));
  const orthoHospitals = hospitals.filter((h) => orthoHospitalSlugs.has(h.slug));
