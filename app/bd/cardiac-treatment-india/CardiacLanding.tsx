...
const categoryIcons = [HeartPulse, Activity, HeartHandshake, Waves, Gauge, Zap, Activity, HeartPulse, Waves, Gauge, HeartHandshake, Search];

function track(event: string, extra?: Record<string, unknown>) {
  if (typeof window !== "undefined" && (window as any).dataLayer) {
    (window as any).dataLayer.push({ event, source: "cardiac-treatment-india", ...extra });
  }
}
...
export default function CardiacLanding() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // fail safe by showing the modal
    }
  ...
  const heroWhatsapp = whatsappHref("Hello TrueCare, I'm interested in cardiac treatment in India.");
  ...
  return (
    <div>
      ...
      <div className="border-b border-navy-100/60 bg-navy-50/60 py-2.5 dark:border-white/10 dark:bg-white/5">
        <Container className="flex flex-wrap items-center justify-between gap-2 text-sm">
          <span className="flex items-center gap-2 font-medium text-navy-500 dark:text-white/80">
            <HeartPulse size={15} className="text-primary-500" /> {t.bdBadge}
          </span>
        </Container>
      </div>
        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <Button href="#report-form" size="lg" onClick={() => track("CTA_click", { cta: "hero_primary" })}>
                {t.hero.ctaPrimary}
              </Button>
              <a
                href={heroWhatsapp}
        ...
        <Container>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {t.trustStrip.items.map((item, i) => {
              const icons = [HeartHandshake, Activity, Building2, ClipboardCheck];
      ...
      {/* Cardiac treatment categories */}
      <section className="py-16 sm:py-20">
        <Container>
          <h2 className="mb-3 text-center font-display text-2xl font-bold text-navy-500 dark:text-white sm:text-3xl">
            {t.categories.title}
          </h2>
      ...
      {/* Featured cardiac specialists */}
      {featuredDoctors.length > 0 && (
        <section className="bg-surface-soft py-16 dark:bg-surface-darkSoft sm:py-20">
          <Container>
            <h2 className="mb-2 text-center font-display text-2xl font-bold text-navy-500 dark:text-white sm:text-3xl">
              ...
              {featuredDoctors.map((doctor) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 flex items-center justify-center gap-1 text-center text-xs text-primary-600 dark:text-primary-300">
                      <Stethoscope size={12} className="flex-shrink-0" /> {doctor.specialty}
                    </p>
                    <p className="mt-1 text-xs text-navy-300 dark:text-white/60">{doctor.hospital}</p>
                    {doctor.qualifications && (
                      <p className="mt-2 flex items-start justify-center gap-1 text-left text-[11px] leading-snug text-navy-300 dark:text-white/50">
                        <GraduationCap size={11} className="mt-0.5 flex-shrink-0" />
                        <span className="line-clamp-3">{doctor.qualifications}</span>
                      </p>
                    )}
                ...
                >
                  ...
                  <div className="p-6">
                    <div className="flex items-start gap-3">
                      <Building2 size={20} className="mt-0.5 flex-shrink-0 text-primary-500" />
                      <div>
          ...
          <div className="mx-auto max-w-2xl space-y-6">
            {t.education.blocks.map((block) => (
              <div key={block.h}>
                ...
                <p className="mt-1.5 text-sm leading-relaxed text-navy-300 dark:text-white/60">{block.p}</p>
              </div>
            ))}
            <p className="rounded-xl bg-white p-4 text-xs text-navy-400 dark:bg-white/5 dark:text-white/50">
          ...
          <div className="mx-auto max-w-2xl text-center">
            ...
            <div className="mx-auto mt-5 grid max-w-lg grid-cols-2 gap-2 text-left sm:grid-cols-2">
              {t.cost.factors.map((f) => (
                <span
                  key={f}
                  className="flex items-center gap-2 rounded-lg bg-navy-50/60 px-3 py-2 text-xs text-navy-400 shadow-card dark:bg-white/5 dark:text-white/60"
                >
      ...
      {/* Report submission */}
      <section id="report-form" className="bg-surface-soft py-16 dark:bg-surface-darkSoft sm:py-20">
        <Container>
          <div className="mx-auto max-w-2xl">
            <div className="text-center">
              ...
              <p className="mx-auto mt-4 max-w-xl text-sm text-navy-300 dark:text-white/60">{t.reports.body}</p>
              <div className="mx-auto mt-4 flex max-w-xl flex-wrap justify-center gap-2">
                {t.reports.examples.map((e) => (
                  <span
                    key={e}
                    className="rounded-full bg-white px-3 py-1 text-xs font-medium text-navy-400 shadow-card dark:bg-white/5 dark:text-white/60"
                  >
                    {e}
                  </span>
                ))}
            ...
            <div className="mt-8 rounded-2xl border border-navy-100/70 bg-white p-6 shadow-card dark:border-white/10 dark:bg-white/5 sm:p-8">
              <ReportForm whatsappNumber={WHATSAPP_NUMBER} />
            </div>
          </div>
        </Container>
      </section>
                ...
                >
                  ...
                  <ul className="mt-3 space-y-1.5">
                    {phase.items.map((item) => (
                      <li key={item} className="flex items-start gap-1.5 text-xs text-navy-300 dark:text-white/60">
