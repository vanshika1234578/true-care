...
export default function ReportForm({
    ...
    try {
      ...
      const res = await fetch("/api/report-submission", {
        ...
        body: JSON.stringify({
          ...
          condition,
          timeline,
          source: "cardiac-treatment-india",
          reportUrls,
          lang: "en",
          turnstileToken,
        }),
      ...
      if (typeof window !== "undefined" && (window as any).dataLayer) {
        (window as any).dataLayer.push({ event: "report_form_submitted", source: "cardiac-treatment-india" });
        (window as any).dataLayer.push({ event: "qualified_report_submission", source: "cardiac-treatment-india" });
      }
    ...
    return (
      <div className="flex flex-col items-center rounded-2xl border border-teal-200 bg-teal-50 p-8 text-center dark:border-teal-500/20 dark:bg-teal-500/10 sm:p-10">
        ...
        <a
          ...
          onClick={() => {
            if (typeof window !== "undefined" && (window as any).dataLayer) {
              (window as any).dataLayer.push({ event: "whatsapp_click", location: "report_form_success", source: "cardiac-treatment-india" });
            }
          }}
  ...
  return (
    <form
      ...
      onFocus={() => {
        if (typeof window !== "undefined" && (window as any).dataLayer) {
          (window as any).dataLayer.push({ event: "report_form_started", source: "cardiac-treatment-india" });
        }
      }}
      className="space-y-5"
      noValidate
